...
<Update label="July 17, 2026" tags={["Agent", "Performance", "Dashboard"]}>
  ...
  **Upgrading the agent:** run `composer update nightowl/agent`, then in the monitored app run `php artisan nightowl:migrate` and restart the agent. To convert an existing install's raw tables to partitioning, also run `php artisan nightowl:partition` — do this in a quiet window if your logs table is large (it rewrites that one table under a lock). See the [agent changelog (1.3.0)](https://github.com/lemed99/nightowl-agent/blob/main/CHANGELOG.md#130---2026-07-17) for full details and tuning options.
</Update>
