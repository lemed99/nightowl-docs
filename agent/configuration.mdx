...
| `NIGHTOWL_INGEST_URI`       | `127.0.0.1:2407` | Where your **app transmits** telemetry — the agent's `host:port` as seen from your app. Defaults to the loopback listener (co-located agent). Set it to the agent's private address when the app and agent run on different hosts — required for serverless hosts like [Laravel Vapor](/agent/vapor). A bare host with no port falls back to `NIGHTOWL_AGENT_PORT`. Mirrors nightwatch's `NIGHTWATCH_INGEST_URI`. |
| `NIGHTOWL_INGEST_TIMEOUT`   | `0.5`       | Connect + write timeout (seconds) for transmitting to the agent. The default suits a loopback agent; raise it when the agent is a network hop away (e.g. Vapor → EC2). |
