...
This mirrors how [Laravel Nightwatch runs on Vapor](https://nightwatch.laravel.com/docs/guides/vapor) — the agent lives on a persistent instance, and the serverless app transmits to it over the private network. The same split works for ordinary servers too — see [Dedicated monitoring server](/agent/remote-agent).
