...
Every action in the UI — status change, priority edit, comment, bulk update — has a direct API equivalent under `/api/data/{app}/issues`. AI assistants can do the same through the [MCP server](/dashboard/mcp-server), which wraps those endpoints with auditable tokens and a fixed tool surface.
