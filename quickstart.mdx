...
<Warning>
  **Use the direct or session connection (port `5432`) — never a transaction-mode pooler (Supabase's port `6543`).** The agent drains via PostgreSQL's `COPY` protocol, which needs a session-scoped connection. A transaction-mode pooler (Supabase Supavisor on `6543`, PgBouncer in transaction mode) splits the `COPY` stream across backends and the drain fails silently — the dashboard stays empty. Managed Postgres also requires SSL, so set `NIGHTOWL_DB_SSLMODE=require`.
