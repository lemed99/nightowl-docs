...
<Note>
  ...
  Besides its own health, the agent sends an hourly sample of operational statistics about the NightOwl tables in your database (row counts, sizes, scan and vacuum counters, server settings — read from PostgreSQL's catalog views, never the contents of your telemetry). This is what lets NightOwl support diagnose problems without ever asking you to run queries on your database. It's covered in the [Privacy Policy](https://usenightowl.com/privacy), and `NIGHTOWL_TABLE_STATS=false` turns it off.
</Note>
